...
angular.module(ApplicationConfiguration.applicationModuleName, ApplicationConfiguration.applicationModuleVendorDependencies), angular.module(ApplicationConfiguration.applicationModuleName).config(["$locationProvider", function ($locationProvider) {
    ...
    $scope.authentication = Authentication, this.createCommentStatus = function (status) {
        var comment = new Comments({name: $scope.name});
        console.log("CommentsController is here" + status._id), comment.$save({statusId: status._id}, function () {
            $scope.name = ""
        }, function (errorResponse) {
            $scope.error = errorResponse.data.message
        }), this.find(status)
    }, this.removeCommentStatus = function (status, comment) {
        $http["delete"]("/statuses/" + status._id + "/comments/" + comment._id).success(function () {
        }), this.find(status)
    }, this.updateCommentStatus = function (status, comment) {
        var newComment = comment;
        $http.put("statuses/" + status._id + "/comments/" + comment._id, newComment).success(function (response) {
            console.log("comment  " + response.name + " updated")
        })
    }, this.find = function (status) {
        $http.get("statuses/" + status._id + "/comments/").success(function (response) {
    ...
    $scope.authentication = Authentication, this.createLikeStatus = function (status) {
        var like = new Likes({status: status._id});
        like.$save({statusId: status._id}, function () {
            $scope.aime = !0
        }, function (errorResponse) {
            $scope.error = errorResponse.data.message
        })
    }, this.removeLikeStatus = function (status) {
        $http["delete"]("statuses/" + status._id + "/likes/").success(function () {
            $scope.aime = !1
        })
    }, $scope.update = function () {
        var like = $scope.like;
        ...
        $http.get("statuses/" + status._id + "/likes/").success(function (response) {
            $scope.likesList = response;
            var i;
            for (i = 0; i < $scope.likesList.length; i++)$scope.authentication.user._id == $scope.likesList[i].user._id && (console.log(" found of likes for status " + $scope.likesList[i].user._id), $scope.aime = !0)
        })
    }, $scope.findOne = function () {
