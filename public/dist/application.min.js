...
var ApplicationConfiguration = function () {
    var applicationModuleName = "mean", applicationModuleVendorDependencies = ["ngResource", "ngAnimate", "ui.router", "ui.bootstrap", "ui.utils"], registerModule = function (moduleName, dependencies) {
...
angular.module(ApplicationConfiguration.applicationModuleName, ApplicationConfiguration.applicationModuleVendorDependencies), angular.module(ApplicationConfiguration.applicationModuleName).config(["$locationProvider", function ($locationProvider) {
...
}), ApplicationConfiguration.registerModule("articles"), ApplicationConfiguration.registerModule("contenus"), ApplicationConfiguration.registerModule("core"), ApplicationConfiguration.registerModule("pub-imags"), ApplicationConfiguration.registerModule("pub-videos"), ApplicationConfiguration.registerModule("statuses"), ApplicationConfiguration.registerModule("users"), angular.module("articles").run(["Menus", function (Menus) {
...
}]), angular.module("pub-imags").controller("PubImagsController", ["$scope", "$stateParams", "$location", "Authentication", "PubImags", function ($scope, $stateParams, $location, Authentication, PubImags) {
    $scope.authentication = Authentication;
    var image;
    $scope.load = function () {
        var fileInput = document.querySelector("#file"), reader = new FileReader;
        fileInput.onchange = function () {
            document.getElementById("namefile").value = fileInput.files[0].name, reader.onload = function () {
                document.getElementById("image").style.width = "242px", document.getElementById("image").style.height = "200px", document.getElementById("image").src = reader.result, image = reader.result
            }, reader.readAsDataURL(fileInput.files[0])
        }
    }, $scope.create = function () {
        var pubImag = new PubImags({name: image, comment: this.comment});
        pubImag.$save(function (response) {
            $location.path("pub-imags/" + response._id), $scope.name = ""
        }, function (errorResponse) {
            $scope.error = errorResponse.data.message
        })
    ...
    $scope.authentication = Authentication, $scope.create = function () {
        var pubVideo = new PubVideos({name: this.name});
        pubVideo.$save(function (response) {
            $location.path("pub-videos/" + response._id), $scope.name = ""
        }, function (errorResponse) {
            $scope.error = errorResponse.data.message
        })
