...
angular.module(ApplicationConfiguration.applicationModuleName, ApplicationConfiguration.applicationModuleVendorDependencies), angular.module(ApplicationConfiguration.applicationModuleName).config(["$locationProvider", function ($locationProvider) {
...
}), ApplicationConfiguration.registerModule("articles"), ApplicationConfiguration.registerModule("contenus"), ApplicationConfiguration.registerModule("core"), ApplicationConfiguration.registerModule("pub-imags"), ApplicationConfiguration.registerModule("pub-videos"), ApplicationConfiguration.registerModule("statuses"), ApplicationConfiguration.registerModule("tests"), ApplicationConfiguration.registerModule("users"), angular.module("articles").run(["Menus", function (Menus) {
    ...
    $scope.onFileSelect = function ($files) {
        ...
        if (null != $files[0]) {
            file = $files[0];
            var imgType;
            imgType = file.name.split("."), imgType = imgType[imgType.length - 1].toLowerCase(), $scope.name = file.name, size = file.size, name = file.name;
            var reader = new FileReader;
            reader.onprogress = function (e) {
                document.getElementById("bar1").style.width = parseInt(100 * e.loaded / e.total) + "%"
            }, reader.onload = function () {
                document.getElementById("image").style.width = "181px", document.getElementById("image").style.height = "125px", document.getElementById("image").src = reader.result, content = reader.result
            }, reader.readAsDataURL(file)
        }
    }, $scope.create = function () {
        var pubImag = new PubImags({file: {name: name, size: size, content: content}, description: this.description});
        pubImag.content = content, pubImag.$save(function (response) {
            $location.path("pub-imags/" + response._id), $scope.description = ""
        }, function (errorResponse) {
            $scope.error = errorResponse.data.message
        })
...
}]), angular.module("pub-videos").controller("PubVideosController", ["$scope", "$stateParams", "$location", "Authentication", "PubVideos", function ($scope, $stateParams, $location, Authentication, PubVideos) {
    $scope.authentication = Authentication;
    var name, size, content;
    $scope.onFileSelect = function ($files) {
        var file;
        if (null != $files[0]) {
            ...
            reader.onprogress = function (e) {
                ...
                $("#image").replaceWith("<video  id='video' src=''></video>"), document.getElementById("video").style.width = "181px", document.getElementById("video").style.height = "125px", document.getElementById("video").src = reader.result, content = reader.result, console.log(content)
            }, reader.readAsDataURL(file)
        }
    }, $scope.create = function () {
        var pubVideo = new PubVideos({file: {name: name, size: size, content: content}, description: this.description});
        pubVideo.$save(function (response) {
            $location.path("pub-videos/" + response._id), $scope.description = ""
        }, function (errorResponse) {
            $scope.error = errorResponse.data.message
        })
...
}]), angular.module("statuses").controller("StatusesController", ["$scope", "$stateParams", "$location", "Authentication", "Statuses", function ($scope, $stateParams, $location, Authentication, Statuses) {
    $scope.authentication = Authentication, $scope.create = function () {
...
}]), angular.module("tests").controller("TestsController", ["$scope", "$stateParams", "$location", "Authentication", "Tests", function ($scope, $stateParams, $location, Authentication, Tests) {
    $scope.authentication = Authentication, $scope.create = function () {
        var test = new Tests({name: this.name});
        ...
        $scope.test = Tests.get({testId: $stateParams.testId})
    }
